...
var basePath = './../../../';
var normalFolder = basePath + 'script_generator/';

var settings = require(normalFolder + 'settings.json');                              // The settings file
var scriptDir = normalFolder + settings.scriptDir + '/';                               // The directory where dota scripts are placed
...
var customDir = normalFolder + settings.customDir;                                     // The directory where our mods are read from, to be merged in
var customAbilitiesLocation = normalFolder + '../src/scripts/npc/npc_abilities_custom.txt'
var langDir = normalFolder + '../src/localization/';
...
// Lets do some fixups
function lazyMerge(kv1, kv2, fixups, outLocation, callback, encoding1, encoding2, outputEncoding) {
    if(encoding1 == null) encoding1 = 'ascii';
    if(encoding2 == null) encoding2 = 'ascii';
    if(outputEncoding == null) outputEncoding = 'ascii';
    ...
    var onReadData = function() {
        ...
        if(++totalDone != 2) return;

        // Do a lazy replace for the fixups in kv2
        for(var searchFor in fixups) {
            var replaceWith = fixups[searchFor];
            kv2Data = kv2Data.replace(new RegExp(searchFor, 'g'), replaceWith);
        }

        var k1 = parseKV(kv1Data);
        var k2 = parseKV(kv2Data);

        var mergeIt = function(main, sub) {
            for(var key in main) {
                if(sub[key] == null) {
                    sub[key] = main[key];
                } else {
                    if(typeof main[key] == 'object' && typeof sub[key] == 'object') {
                        mergeIt(main[key], sub[key]);
                    } else {
                        sub[key] = main[key];
                    }
                }
        ...
        if(outputEncoding != 'ascii') {
            output = specialChar + output;
        }

        fs.writeFile(outLocation, output, outputEncoding, function(err) {
            if(err) throw err;
    ...
    fs.readFile(kv1, encoding1, function(err, data) {
        if(err) throw err;

        kv1Data = data;

        if(encoding1 != 'ascii') {
            specialChar = kv1Data.substr(0, 1);
            kv1Data = kv1Data.substr(1);
        }

        onReadData();
    });
    fs.readFile(kv2, encoding2, function(err, data) {
        if(err) throw err;

        kv2Data = data;

        if(encoding2 != 'ascii') {
            specialChar = kv2Data.substr(0, 1);
            kv2Data = kv2Data.substr(1);
        }
...
// Merge items
lazyMerge(
    basePath + 'dota/game/scripts/npc/npc_units_custom.txt',
    basePath + 'import/dota_imba/scripts/npc/npc_units_custom.txt',
    {
        '"items/': '"imba_imported/items/',
        '"internal/': '"imba_imported/internal/',
        '"hero/': '"imba_imported/hero/'
    },
    basePath + 'dota/extended/game/scripts/npc/npc_units_custom.txt',
    function() {
        console.log('merged units successfully');
    }
);

lazyMerge(
    basePath + 'dota/game/scripts/npc/npc_items_custom.txt',
    basePath + 'import/dota_imba/scripts/npc/npc_items_custom.txt',
    {
        '"items/': '"imba_imported/items/',
        '"internal/': '"imba_imported/internal/',
        '"hero/': '"imba_imported/hero/'
    },
    basePath + 'dota/extended/game/scripts/npc/npc_items_custom.txt',
    function() {
        console.log('merged items successfully');
    }
);

lazyMerge(
    basePath + 'dota/game/scripts/npc/npc_abilities_custom.txt',
    basePath + 'import/dota_imba/scripts/npc/npc_abilities_custom.txt',
    {
        '"items/': '"imba_imported/items/',
        '"internal/': '"imba_imported/internal/',
        '"hero/': '"imba_imported/hero/'
    },
    basePath + 'dota/extended/game/scripts/npc/npc_abilities_custom.txt',
    function() {
        console.log('merged custom abilities successfully');
    }
);

lazyMerge(
    basePath + 'dota/game/resource/addon_english.txt',
    basePath + 'import/dota_imba/resource/addon_english.txt',
    {},
    basePath + 'dota/extended/game/resource/addon_english.txt',
    function() {
        console.log('merged language files successfully');
    },
    'ucs2', 'ucs2', 'ucs2'
);
